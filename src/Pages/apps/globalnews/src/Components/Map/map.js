  ...
  useMapEvents({ // a hook from leaflet library
    click : (e) => {
      ...
      .then(response => {
        ...
        if(response.length < 2){ // If no usable results from google api
      ...
      function placeNamefromTypes(category){
        ...
        return names
          ...
          (async function() {
            ...
            const countryOrNonNationalName = await (country === null) ? naturalFeature : country
            ...
            for(let x = 0; x < allLocalPlaceNames.length; x++ ){
              ...
              .then(response => {
                if(response.length > 0){ // if news found
                  ...
                  if(country === null){ // passing data for body of water
